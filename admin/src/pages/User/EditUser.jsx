...
import { apiRequest } from '../../services/apiv1';
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const response = await apiRequest(`user/${uid}`, 'PUT', updatedUser);
  ...
  const { uid } = useParams(); // Get UID from route params
  console.log(uid)

  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await apiRequest(`user/${uid}`, 'GET');
        const user = response;
